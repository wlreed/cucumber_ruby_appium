...
# meta-automation formatter
# calls to puts are changed to split_puts which stuffs the output into
# the log files
#
...
module MetaLogging
  ...
  class MetaPretty
    ...
    attr_reader :config, :options, :current_feature_uri, :current_scenario_outline, :current_examples, :current_test_case, :in_scenario_outline,
    ...
    def initialize(config)
      ...
      @issues = Cucumber::Formatter::ConsoleIssues.new(config, @ast_lookup)
      @first_feature = true
      @current_feature_uri = ''
    ...
    def on_test_case_finished(event)
      @total_duration += Cucumber::Formatter::DurationExtractor.new(event.result).result_duration
    ...
    def print_step_output
      @test_step_output.each { |message| @io.split_puts(indent(format_string(message, :tag), 6)) }
      @test_step_output = []
    end
    ...
    def print_language_comment(feature_line)
      gherkin_source.split("\n")[0..feature_line].each do |line|
        @io.split_puts(format_string(line, :comment)) if /# *language *:/ =~ line
      end
    end
    ...
    def print_comments(up_to_line, indent_amount)
      ...
      comments[@next_comment_to_be_printed..].each do |comment|
        if comment.location.line <= up_to_line
          @io.split_puts(indent(format_string(comment.text.strip, :comment), indent_amount))
    ...
    def print_tags(tags, indent_amount)
      ...
      @io.split_puts(indent(tags.map { |tag| format_string(tag.name, :tag) }.join(' '), indent_amount))
    end
    ...
    def print_doc_string(content, status, indent_amount)
      s = indent(%("""\n#{content}\n"""), indent_amount)
      s = s.split("\n").map { |l| l =~ /^\s+$/ ? '' : l }.join("\n")
      @io.split_puts(format_string(s, status))
    end
